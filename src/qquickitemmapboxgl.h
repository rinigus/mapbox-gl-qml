...
class QQuickItemMapboxGL : public QQuickItem {
    ...
    Q_PROPERTY(QRectF margins READ margins WRITE setMargins NOTIFY
                   marginsChanged) /// see comments below on interpretation of RectF
    ...
    Q_PROPERTY(qreal metersPerPixelTolerance READ metersPerPixelTolerance WRITE
                   setMetersPerPixelTolerance NOTIFY metersPerPixelToleranceChanged)

    // error
    Q_PROPERTY(QString errorString READ errorString NOTIFY errorChanged)

    // for internal use. used by map area to notify that the gesture is in progress
    Q_PROPERTY(bool gestureInProgress READ gestureInProgress WRITE setGestureInProgress NOTIFY
    ...
    Q_PROPERTY(QString cacheDatabasePath READ cacheDatabasePath WRITE setCacheDatabasePath NOTIFY
                   cacheDatabasePathChanged)
    Q_PROPERTY(int cacheDatabaseMaximalSize READ cacheDatabaseMaximalSize WRITE
                   setCacheDatabaseMaximalSize NOTIFY cacheDatabaseMaximalSizeChanged)
    Q_PROPERTY(bool cacheDatabaseDefaultPath READ cacheDatabaseDefaultPath WRITE
                   setCacheDatabaseDefaultPath NOTIFY cacheDatabaseDefaultPathChanged)
    Q_PROPERTY(bool cacheDatabaseStoreSettings READ cacheDatabaseStoreSettings WRITE
                   setCacheDatabaseStoreSettings NOTIFY cacheDatabaseStoreSettingsChanged)
  ...
  public:
    ...
    Q_INVOKABLE void addSourcePoint(const QString &sourceID, const QGeoCoordinate &coordinate,
                                    const QString &name = QString());
    Q_INVOKABLE void addSourcePoint(const QString &sourceID, qreal latitude, qreal longitude,
                                    const QString &name = QString());

    /// in addSourcePoints and addSourceLine, coordinates are given as a list of QGeoCoordinate
    Q_INVOKABLE void addSourcePoints(const QString &sourceID, const QVariantList &coordinates,
                                     const QVariantList &names = QVariantList());
    Q_INVOKABLE void addSourceLine(const QString &sourceID, const QVariantList &coordinates,
                                   const QString &name = QString());
    ...
    Q_INVOKABLE void updateSourcePoint(const QString &sourceID, const QGeoCoordinate &coordinate,
                                       const QString &name = QString());
    Q_INVOKABLE void updateSourcePoint(const QString &sourceID, qreal latitude, qreal longitude,
                                       const QString &name = QString());

    /// in updateSourcePoints and updateSourceLine, coordinates are given as a list of
    /// QGeoCoordinate
    Q_INVOKABLE void updateSourcePoints(const QString &sourceID, const QVariantList &coordinates,
                                        const QVariantList &names = QVariantList());
    Q_INVOKABLE void updateSourceLine(const QString &sourceID, const QVariantList &coordinates,
                                      const QString &name = QString());

    Q_INVOKABLE void removeSource(const QString &sourceID);

    Q_INVOKABLE void addLayer(const QString &id, const QVariantMap &params,
                              const QString &before = QString());
    Q_INVOKABLE void removeLayer(const QString &id);

    Q_INVOKABLE void addImage(const QString &name, const QImage &sprite);
    Q_INVOKABLE bool addImagePath(
        const QString &name, const QString &path, const int svgX = 0,
        const int svgY = 0); ///< Add image using image file path or url starting with file://
    Q_INVOKABLE void removeImage(const QString &name);

    Q_INVOKABLE void setLayoutProperty(const QString &layer, const QString &property,
                                       const QVariant &value);
    Q_INVOKABLE void setLayoutPropertyList(const QString &layer, const QString &property,
                                           const QVariantList &value);
    Q_INVOKABLE void setPaintProperty(const QString &layer, const QString &property,
                                      const QVariant &value);
    Q_INVOKABLE void setPaintPropertyList(const QString &layer, const QString &property,
                                          const QVariantList &value);
  ...
  signals:
    ...
    void replyCoordinateForPixel(const QPointF pixel, QGeoCoordinate geocoordinate,
                                 qreal degLatPerPixel, qreal degLonPerPixel, const QVariant &tag);
  ...
  private:
    ...
    std::string resourceTransform(
        const std::string &url); ///< Use resource transform API to change requested URL
  ...
  private:
    ...
    bool m_block_data_until_loaded{
        true}; ///< Blocks loading of additional data until base map is loaded
    bool m_finalize_data_loading{
        true}; ///< Used to load additional data when the base map is fully loaded
